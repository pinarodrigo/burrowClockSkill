...
const webService = "https://t7bes9gh71.execute-api.eu-central-1.amazonaws.com/dev/route/";
...
async function callLocalizar(nombre, context) {
    ...
    let duration = elements[2];

    console.log(JSON.stringify(context.event.session.user));
    console.log(JSON.stringify(context.event.context.System.device));


    if (poi == 'undefined' || poi == "") {
        let distanceInMinutes = duration / 60;
        console.log("Distance: " + distance + ", time: " + distanceInMinutes)
        speechText = nombre + " está a " + Math.round(distance) + " metros de la casa, aproximadamente a " + Math.round(distanceInMinutes) + " minutos caminando";
    } else {
